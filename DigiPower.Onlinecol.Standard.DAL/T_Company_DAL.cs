...
namespace DigiPower.Onlinecol.Standard.DAL {
    ...
    public class T_Company_DAL {
        ...
        /// <returns></returns>
        public DataSet GetMyChildCompany(string CompanyType, string CompanyName, string ParentCompanyID, string MyCompanyID) {//比较复杂
            StringBuilder strSql = new StringBuilder();
        ...
        /// <param name="MyCompanyID"></param>
        /// <returns></returns>
        public DataSet GetMyChildCompany(string CompanyType, string CompanyName, string ParentCompanyID, string MyCompanyID, string Area_Code) {//比较复杂
            StringBuilder strSql = new StringBuilder();
        ...
        public bool DeleteCompanyOther(string CompanyID) {
            try {
                if (CompanyID.Trim().Length > 0) {
                    ...
                    listSql.Add("delete from T_COMPANY where COMPANYID=" + CompanyID);                         //删除公司
                    listSql.Add("delete from T_SINGLEPROJECTCOMPANY where COMPANYID=" + CompanyID);            //删除工程公司表
                    listSql.Add("delete from T_SingleProjectUser where UserID in(SELECT UserID from T_UsersInfo where CompanyID=" + CompanyID + ")");  //删除公司用户表
                    listSql.Add("delete from T_USERSINFO where COMPANYID=" + CompanyID);                       //删除用户表     
                    listSql.Add("delete from T_FileList where COMPANYID=" + CompanyID);                        //删除文件表
