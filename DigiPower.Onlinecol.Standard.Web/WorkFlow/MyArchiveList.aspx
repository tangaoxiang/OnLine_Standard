<%@ Page Language="C#" MasterPageFile="~/SiteParent.Master" AutoEventWireup="true" CodeBehind="MyArchiveList.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.WorkFlow.MyArchiveList" Title="单位案卷管理" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Head" runat="server">
    <script language="javascript" type="text/javascript">
        ...
        function btnEdit() {
            if (Common.isCheckBoxOneChecked()) {
                var keyList = Common.getChkOneKeyToArray();
                var url = 'MyArchiveAdd.aspx?action=' + PageState.Edit + '&id=' + keyList[0] + '&rn=' + Math.random();
                Common.fnLayerOpen('案卷信息-修改', "550px", "450px", url, true, "案卷信息修改成功!");
            }
        }
        //删除
        function btnDel() {
            if (Common.isCheckBoxMoreChecked()) {
                layer.confirm('确定要删除选中案卷信息?', function () {
                    ...
                    MyArchiveList.DeleteArchiveMore(keyList).value
        ...
        function batchSaveIndex() {
            ...
            if (checkedLen > 0) {
                $("input[type='checkbox']:checked").each(function () {
                    ...
                    MyArchiveList.UpdateArchiveXH(keyList[0], $(tdAll).eq(6).children(0).val()).value;
                });
        ...
        function btnArchiveToFileList() {
            if (Common.isCheckBoxOneChecked()) {
                var keyList = Common.getChkOneKeyToArray();
                var url = 'MyArchiveToFileList.aspx?action=' + PageState.Edit + '&ArchiveID=' + keyList[0] + '&rn=' + Math.random();
        ...
        function btnUpdateFileOrderIndex() {
            ...
            if (checkedLen > 0) {
                $("input[type='checkbox']:checked").each(function () {
                    var keyList = ($(this).val() + ',').split(',');
                    MyArchiveList.updateFileOrderIndexByArchiveID(keyList[0]).value;
                });
        ...
        <div class="content-box-content">
            <div class="tab-content default-tab" id="tab1">
                <div class="anl" id="anlButtonList">
                    ...
                    <input type="button" id="MrcBtnEdit" value="修改" title="修改案卷详细信息" onclick="btnEdit()" />
                    ...
                    <input type="button" id="MrcBtnPrintJLML" value="打印卷内目录" onclick="btnEdit(2)" />
                    <input type="button" id="MrcBtnPrintBKB" value="打印卷内备考表" onclick="btnEdit(3)" />
                </div>
                <table>
                    <thead>
                        <tr>
