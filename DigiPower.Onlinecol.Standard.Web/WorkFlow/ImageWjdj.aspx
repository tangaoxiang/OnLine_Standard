...
<asp:Content ID="Content1" ContentPlaceHolderID="Head" runat="server">
    ...
    <script type="text/javascript">
            ...
            if (Common.isCheckBoxOneChecked()) {
                ...
                if (keyList[5].length > 1) {
                    ...
                    var url = "BatChUpload/BatchUpload.aspx?FileListID=" + FileListID + "&SingleProjectID=" +
                        GetSingleProjectID() + "rn=" + Math.random();
                    layer.open({
                        type: 2,
        ...
        function lookPDF(isclick, bh, filelistID) {
            if (isclick == 1) {
            ...
            if (checkedLen > 0) {
                $("input[type='checkbox']:checked").each(function () {
                    ...
                    if (keyList[1].toLocaleLowerCase() == 'false' || keyList[1] == '0') { //只保存非节点数据     
                        ...
                        var oldTitle = $(tdAll).eq(1).children(0).val();
                        var oldPsz = $(tdAll).eq(2).children(0).val();
                        var oldPssj = $(tdAll).eq(3).children(0).val();
                        var oldPsdd = $(tdAll).eq(4).children(0).val();
                        var oldOrderIndex = $(tdAll).eq(5).children(0).val();
                        ...
                        <asp:Repeater ID="rpData" runat="server">
                            <ItemTemplate>
                                <tr class='<%#(Container.ItemIndex%2==0)?"bg01":""%>'>
                                    <td <%# getTdColorForFileStatus(Eval("Status"))%>>
                                        <input type="checkbox" name="checkOne"
                                            value='<%# Eval("FileListID")%>,<%# Eval("IsFolder")%>,
                                                <%# Eval("recID")%>,<%# Eval("PID")%>,<%# Eval("DefaultCompanyType")%>,<%# Eval("BH")%>' />
                                    </td>
                                    <td <%# getTdColorForFileStatus(Eval("Status"))%>>
                                        <%# Eval("bh")%>
                                    </td>
                                    ...
                                    <td>
                                        <%# getEfileImage(Eval("IsFolder"),Eval("BH").ToString().Trim(), Eval("FileListID").ToString())%>
                                    </td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
