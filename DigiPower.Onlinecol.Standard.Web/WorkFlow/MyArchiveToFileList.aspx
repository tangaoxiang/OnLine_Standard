<%@ Page Language="C#" MasterPageFile="~/SiteParent.Master" AutoEventWireup="true" CodeBehind="MyArchiveToFileList.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.WorkFlow.MyArchiveToFileList" Title="单位案卷-FileList" %>
    ...
    <script type="text/javascript">
        ...
        function lookPDF(isFolder, bh, filelistID, SingleProjectID) {
            if (Number(isFolder) == 0) {
                if (MyArchiveToFileList.CheckPdfFileExistsForFileListID(Number(filelistID)).value) {
        ...
        function batchUpdateXH() {
            ...
            if (checkedLen > 0) {
                $("input[type='checkbox']:checked").each(function () {
                    ...
                    MyArchiveToFileList.UpdateFileOrderIndex(keyList[0], $(tdAll).eq(7).children(0).val()).value;
                });
                        ...
                        <asp:Repeater ID="rpData" runat="server">
                            <ItemTemplate>
                                <tr class='<%#(Container.ItemIndex%2==0)?"bg01":""%>'>
                                    ...
                                    <td>
                                        <input type="text" value="<%# Eval("MyOrderIndex")%>" name="OrderIndex" class="regedit_input" maxlength="4" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" />
                                    </td>
                                    <td><%# 
