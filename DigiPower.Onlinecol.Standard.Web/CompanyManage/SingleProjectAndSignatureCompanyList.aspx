<%@ Page Language="C#" MasterPageFile="~/SiteParent.Master" AutoEventWireup="true" CodeBehind="SingleProjectAndSignatureCompanyList.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.SystemManage.SingleProjectAndSignatureCompanyList" Title="签章单位挂接" %>
    ...
    <script language="javascript" type="text/javascript">
        ...
        function btnAddRelevanceUser() {
            if (Common.isRadioCheck()) {
                ...
                var url = "ProjectRelevanceSignatureUser.aspx?action=" + PageState.Edit + "&SingleProjectID=" + keyList[0]
                        + "&ProjectType=" + keyList[1] + "&rn=" + Math.random();
                Common.fnLayerOpen("工程签章单位关联"+" 工程编号:"+keyList[5]+" 工程名称:"+keyList[6], "100%", "100%", url, true, "签章单位关联成功!"); 
            }
        } 
        //取消关联
        function btnCancelRelevanceUser() {
            if (Common.isRadioCheck()) {
                ...
                if (Number(keyList[4]) == jsid) {
                    Common.fnLayerAlert('请选择签章单位来取消关联!');
                    return;
                }

                var fileCount = SingleProjectAndSignatureCompanyList.GetSignatureCount(Number(keyList[0]), Number(keyList[3])).value;
                if (fileCount > 0) {
                    Common.fnLayerAlert("该单位已签章文件" + Common.getRedStrongString(fileCount)
                        + "次,不能被取消关联,如需取消,请删除签章记录!");
                } else {
                    layer.confirm('确定要取消关联选中的签章单位?', function () {
                        var len = SingleProjectAndSignatureCompanyList.CancelHookUpCompanyAndUser(keyList[0],
        ...
        <div class="content-box-content">
            <div class="tab-content default-tab" id="tab1">
                <div class="anl" id="anlButtonList">
                    ...
                    <input type="button" id="MrcBtnAddRelevanceUser" title="选择一个工程关联签章单位,如果签章单位已关联,则不会重复关联"
                        onclick="btnAddRelevanceUser()" value="签章单位关联" />
                    <input type="button" id="MrcBtnCancelRelevanceUser" title="如果选择的签章单位已签文件,则不能取消关联"
                        onclick="btnCancelRelevanceUser()" value="取消签章单位关联" />
                </div>
                <table>
                    <thead>
                        <tr>
