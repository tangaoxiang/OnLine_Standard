<%@ Page Language="C#" MasterPageFile="~/SiteParent.Master" AutoEventWireup="true" CodeBehind="CompanyList.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.SystemManage.CompanyList" Title="单位管理" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Head" runat="server">
    <script language="javascript" type="text/javascript">
        ...
        function getCompanyTypeUrl() {
            return "&CompanyType=" + '<%= CompanyType%>';
        }
        //新增
        function btnAdd() {
            var selKey = Common.getRadioKeyToArray();
            var url = '../CompanyManage/CompanyReg1Edit.aspx?action=' + PageState.Add + '&id=0&rn=' + Math.random() + getCompanyTypeUrl();
            if (selKey.length > 0)
                url = '../CompanyManage/CompanyReg1Edit.aspx?action=' + PageState.Add + '&id=' + selKey[0] + '&rn=' +
                    Math.random() + getCompanyTypeUrl();

            Common.fnLayerOpen('单位信息-新增', "100%", "100%", url, true, "单位信息新增成功!");
        }
        //编辑
        function btnEdit() {
            if (Common.isRadioCheck()) {
                var keyList = Common.getRadioKeyToArray();
                var url = '../CompanyManage/CompanyReg1Edit.aspx?action=' + PageState.Edit + '&id=' +
                    keyList[0] + '&rn=' + Math.random() + getCompanyTypeUrl();
                Common.fnLayerOpen('单位信息-修改', "100%", "100%", url, true, "单位信息修改成功!");
            }
        }
        //删除
        function btnDel() {
            if (Common.isRadioCheck()) {
                layer.confirm('确定要删除选中公司?<br/>' + Common.getRedStrongString("注意<br/>1:建设单位不能删除<br/>2:该公司的用户没有登记任何文件则可以删除公司!"), function () {
                    var keyList = Common.getRadioKeyToArray();
                    if (Number(keyList[2]) == Number(<%= DigiPower.Onlinecol.Standard.Web.SystemSet._JSCOMPANYINFO%>)) {
                        Common.fnLayerAlert("建设单位不能删除!");
                        return;
                    }
                    var fileCount = CompanyList.GetFileCount(Number(keyList[0])).value;
                    if (fileCount > 0) {
                        Common.fnLayerAlert("该单位已登记文件" + Common.getRedStrongString(fileCount) + "份,不能删除!");
                    } else {
                        var len = CompanyList.DeleteCompany(keyList[0]).value;
