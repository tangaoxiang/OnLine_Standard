<%@ Page Title="签章单位关联" Language="C#" MasterPageFile="~/SiteParent.Master"
    AutoEventWireup="true" CodeBehind="ProjectRelevanceSignatureUser.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.CompanyManage.ProjectRelevanceSignatureUser" %>
    ...
    <script language="javascript" type="text/javascript">
        function btnAddRelevanceUser() { //单位关联
            if (Common.isCheckBoxMoreChecked()) {
                ...
                $("input[type='checkbox'][name='checkOne']:checked").each(function () {
                    ...
                    var len = ProjectRelevanceSignatureUser.HookUpCompanyAndUser(Number(SingleProjectID), Number(strKey[0]),
        ...
        function btnAddLookCompany() {//单位信息
            if (Common.isCheckBoxOneChecked()) {
                ...
                var url = '../SystemManage/SignatureCompanyAdd.aspx?action=' +
                    PageState.View + '&id=' + keyList[2] + '&rn=' + Math.random() + '&CompanyType=' + keyList[3];
                var height = "100%";
                if (screen.height >= 900) height = "650px";
                window.top.parent.ParentOpenDiv('签章单位信息-预览', url, '900px', height);
            }
        }
    </script>
    <style type="text/css">
...
<asp:Content ID="Content2" ContentPlaceHolderID="Main" runat="server">
    ...
    <div class="main-content">
        <div class="content-box-search">
            <table id="c1">
                <tr>
                    <td class="condition">签章单位角色：</td>
                    <td class="c2">
                        <uc4:ctrlDropDownFileTmpSignatureUserRole ID="ctrlSignatureRole" runat="server" Width="165" />
                    </td>
        ...
        <div class="content-box-content">
            <div class="tab-content default-tab" id="tab1">
                <div class="anl">
                    <input type="button" id="MrcBtnLookCompany" value="签章单位信息" title="点击预览签章单位详细信息" onclick="btnAddLookCompany()" />
                    <input type="button" id="MrcBtnAddRelevanceUser" value="签章单位关联" title="签章单位关联" onclick="btnAddRelevanceUser()" />
