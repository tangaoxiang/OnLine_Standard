<%@ Page Language="C#" MasterPageFile="~/SiteParent.Master" AutoEventWireup="true" CodeBehind="OutHelpCompanyList.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.SystemManage.OutHelpCompanyList" Title="外协单位管理(安监,质监,检测,发改委,财政局等)" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Head" runat="server">
    <script language="javascript" type="text/javascript">
        ...
        function btnAdd() {
            ...
            var url = '../SystemManage/OutHelpCompanyAdd.aspx?action=' + PageState.Add + '&id=0&rn=' + Math.random();
            if (selKey.length > 0)
                url = '../SystemManage/OutHelpCompanyAdd.aspx?action=' + PageState.Add + '&id=' + selKey[0] + '&rn=' +
                    Math.random();

            Common.fnLayerOpen('外协单位信息-新增', "100%", "100%", url, true, "外协单位信息新增成功!");
        }
        //编辑
        function btnEdit() {
            if (Common.isRadioCheck()) {
                var keyList = Common.getRadioKeyToArray();
                var url = '../SystemManage/OutHelpCompanyAdd.aspx?action=' + PageState.Edit + '&id=' +
                    keyList[0] + '&rn=' + Math.random();
                Common.fnLayerOpen('外协单位信息-修改', "100%", "100%", url, true, "外协单位信息修改成功!");
            }
        }
        //删除
        function btnDel() {
            if (Common.isRadioCheck()) {
                layer.confirm('确定要删除选中外协单位?<br/>' + Common.getRedStrongString("注意:外协单位删除后不可恢复!"), function () {
                    var keyList = Common.getRadioKeyToArray();
                    if (Number(keyList[2]) == Number(<%= DigiPower.Onlinecol.Standard.Web.SystemSet._JSCOMPANYINFO%>)) {
                        Common.fnLayerAlert("建设单位不能删除!");
                        return;
                    }
                    var len = OutHelpCompanyList.DeleteCompany(keyList[0]).value;
