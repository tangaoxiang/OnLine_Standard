<%@ Page Language="C#" MasterPageFile="~/SiteSon.Master" AutoEventWireup="true" CodeBehind="OutHelpCompanyAdd.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.SystemManage.OutHelpCompanyAdd" Title="外协单位管理(安监,质监,检测,发改委,财政局等)编辑" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="Main" runat="server">
    ...
    <table class="zpxxxj" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
            <td class="ww">账号角色：</td>
            <td colspan="3">
                <uc4:ctrlDropDownFileTmpSignatureUserRole ID="ctrlOutHelpRole" runat="server" Width="232px" />
            </td>
        </tr>
        <tr>
            <td class="ww">所属文件类别：</td>
            <td colspan="3">
                <uc1:ctrlTextBoxEx ID="txtFileList" labelTitle="所属文件类别" runat="server"
                    ToolTipString="双击选择外协单位所属文件" canEmpty="false" CssClass="cd" MaxLength="200" />
            </td>
        </tr>
        <tr>
            <td class="ww">登录账号：</td>
    ...
    <script language="javascript" type="text/javascript">
        $(document).ready(function () {
            $("#ctl00_Main_txtFileList_TextBox1").dblclick(function () {
                Common.fnLayerOpen("文件类别选择", "380px", "410px", "OutHelpCompanyFileTmpAdd.aspx", false, "");
            });
        });
        function saveSubmit(obj) {
            ...
            if (OutHelpCompanyAdd.CheckCompanyCode('<%= Action%>', Number(<%= ID%>), $CompanyCode.val()).value) {
            ...
            if (OutHelpCompanyAdd.CheckCompanyEmail('<%= Action%>', Number(<%= ID%>), $CompanyEmail.val()).value) {
            ...
            if (OutHelpCompanyAdd.CheckLoginName('<%= Action%>', Number(UserId), LoginNameValue).value) {
            ...
            if (!OutHelpCompanyAdd.CheckPassword(PasswdValue).value) {
