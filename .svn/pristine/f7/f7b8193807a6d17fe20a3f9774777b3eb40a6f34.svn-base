<%@ Page Language="C#" MasterPageFile="~/SiteSon.Master" AutoEventWireup="true" CodeBehind="SignatureCompanyAdd.aspx.cs"
    Inherits="DigiPower.Onlinecol.Standard.Web.SystemManage.SignatureCompanyAdd" Title="签章单位编辑" %>
...
<asp:Content ID="Content4" ContentPlaceHolderID="Main" runat="server">
    ...
    <table class="zpxxxj" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
            <td class="ww">账号角色：</td>
            <td colspan="3">
                <uc4:ctrlDropDownFileTmpSignatureUserRole ID="ctrlSignatureRole" runat="server" Width="208px" />
            </td>
        </tr>
        <tr>
            <td class="ww">登录账号：</td>
    ...
    <script language="javascript" type="text/javascript">
        function saveSubmit(obj) {
            ...
            if (SignatureCompanyAdd.CheckCompanyCode('<%= Action%>', Number(<%= ID%>), $CompanyCode.val()).value) {
            ...
            if (SignatureCompanyAdd.CheckCompanyEmail('<%= Action%>', Number(<%= ID%>), $CompanyEmail.val()).value) {
            ...
            if (SignatureCompanyAdd.CheckLoginName('<%= Action%>', Number(UserId), LoginNameValue).value) {
            ...
            if (!SignatureCompanyAdd.CheckPassword(PasswdValue).value) {
