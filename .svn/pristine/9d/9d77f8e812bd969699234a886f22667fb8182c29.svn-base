...
/*添加自定义的iframe*/
function addCustomIframe(href, id, text) {
    var h = href,
		m = id,
		label = text,
		isHas = false;
...
function navChange() {
    if (!$(this).hasClass("active")) {
        ...
        addTab(this);
        if (k == "home.html") {
            var newSrc = $(".layout-main-body").children().eq(0).attr("src") + "?rn=" + Math.random();
            $(".layout-main-body").children().eq(0).attr("src", newSrc);
        }
    }
}
...
function closePage() {
    ...
    if ($(this).parents(".content-tab").hasClass("active")) {
        ...
        if ($(this).parents(".content-tab").prev(".content-tab").size()) {
            ...
            var n = parseInt($(".tab-nav-content").css("margin-left"));
            var f = (n + cur_width) > 0 ? 0 : n + cur_width;
            $(".tab-nav-content").animate({
                marginLeft: f + "px"
            },
            "fast");
...
function initMenu(menu, parent) {
    for (var i = 0; i < menu.length; i++) {
        ...
        try {
            if (item.isHeader == "1") {
                if (item.id == 1)
                    str = "<li class='menu-header'><i class='icon-font'>&#xe609;</i><span>" + item.name + "</span></li>";
                else
                ...
                if (item.parentId == 1)
                    ...
                    item.icon = "&#xe610;";
                    if (item.parentId == 178)
                        item.icon = "&#xe606;";
                }
                var hrefUrl = item.url;
                if (hrefUrl != "") {
                    if (hrefUrl.indexOf("?") > -1)
                        hrefUrl += "&ModuleID=" + item.id;
                    else
                        hrefUrl = hrefUrl + "?ModuleID=" + item.id;
                }

                if (item.childMenus == "") {
                    str = "<li><a href='" + hrefUrl + "'><i class='icon-font'>" + item.icon + "</i><span>" + item.name + "</span></a></li>";
                    $(parent).append(str);
                } else {
                    str = "<li><a href='" + hrefUrl + "'><i class='icon-font '>" + item.icon + "</i><span>" + item.name + "</span><i class='icon-font icon-right'>&#xe60b;</i></a>";
...
$(function () {
    ...
    $.ajax({
        ...
        data: { FileType: 'getMenu', rd: Math.random() },
        success: function (msg) {
            var jsonDataTree = transData(msg, 'id', 'parentId', 'childMenus');
            //   document.write(JSON.stringify(jsonDataTree));
            initMenu(jsonDataTree, $(".side-menu"));
            $(".side-menu > li").addClass("menu-item");
        },
