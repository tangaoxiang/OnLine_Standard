    ...
    <script language="javascript" type="text/javascript">
        ...
        function btnAddRelevanceUser() {
            if (Common.isRadioCheck()) {  
                ...
                var url = "ProjectRelevanceUser.aspx?action=" + PageState.Edit + "&SingleProjectID=" + keyList[0]
                        + "&ProjectType=" + keyList[1] + "&rn=" + Math.random();
                Common.fnLayerOpen("工程(监理,施工)单位关联"+" 工程编号:"+keyList[5]+" 工程名称:"+keyList[6], "100%", "100%", url, true, "业务单位关联成功!"); 
            }
        } 
        //取消关联
        function btnCancelRelevanceUser() {
            if (Common.isRadioCheck()) {
                ...
                if (Number(keyList[4]) == jsid) {
                    Common.fnLayerAlert('不能取消关联建设单位!');
                    return;
                }
                ...
                if (fileCount > 0) {
                    Common.fnLayerAlert("该单位已登记文件" + Common.getRedStrongString(fileCount)
                        + "份,不能被取消关联,如需取消,请重置文件状态!");
                } else {
                    layer.confirm('确定要取消关联选中的业务单位?', function () {
        ...
        <div class="content-box-content">
            <div class="tab-content default-tab" id="tab1">
                <div class="anl" id="anlButtonList">
                    ...
                    <input type="button" id="MrcBtnAddRelevanceUser" value="业务单位关联"
                        title="选择一个工程关联业务单位,如果单位已关联,则不会重复关联" onclick="btnAddRelevanceUser()" />
                    <input type="button" id="MrcBtnCancelRelevanceUser" value="取消业务单位关联"
                        title="如果选择的业务单位已做了文件,则不能取消关联" onclick="btnCancelRelevanceUser()" />
                </div>
                <table>
                    <thead>
                        <tr>
