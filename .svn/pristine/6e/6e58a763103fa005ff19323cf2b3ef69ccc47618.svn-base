<%@ Page Title="" Language="C#" MasterPageFile="~/SiteParent.Master" AutoEventWireup="true" CodeBehind="ProjectRelevanceUser.aspx.cs" Inherits="DigiPower.Onlinecol.Standard.Web.CompanyManage.ProjectRelevanceUser" %>
    ...
    <script language="javascript" type="text/javascript">
        function btnAddRelevanceUser() { //单位关联
            if (Common.isCheckBoxMoreChecked()) {
                ...
                $("input[type='checkbox'][name='checkOne']:checked").each(function () {
                    ...
                    var len = ProjectRelevanceUser.HookUpCompanyAndUser(Number(SingleProjectID), Number(strKey[0]),
        ...
        function btnAddLookCompany() {//单位信息
            if (Common.isCheckBoxOneChecked()) {
                ...
                var url = '../CompanyManage/CompanyReg1Edit.aspx?action=' +
                    PageState.View + '&id=' + keyList[2] + '&rn=' + Math.random() + '&CompanyType=' + keyList[3];
                var height = "100%";
                if (screen.height >= 900) height = "620px";
                window.top.parent.ParentOpenDiv('单位信息-预览', url, '900px', height);
            }
        }
    </script>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="Main" runat="server">
    ...
    <div class="main-content">
        <div class="content-box-search">
            <table id="c1">
                <tr>
                    <td class="condition">单位类型：</td>
                    <td class="c2">
                        <uc2:ctrlDropDownSystemInfo ID="ddlCompanyType" runat="server"
                            Width="165" CurrentType="CompanyType" />
                    </td>
        ...
        <div class="content-box-content">
            <div class="tab-content default-tab" id="tab1">
                <div class="anl">
                    <input type="button" id="MrcBtnLookCompany" value="单位信息" title="点击预览单位详细信息" onclick="btnAddLookCompany()" />
                    <input type="button" id="MrcBtnAddRelevanceUser" value="单位关联" title="单位关联" onclick="btnAddRelevanceUser()" />
