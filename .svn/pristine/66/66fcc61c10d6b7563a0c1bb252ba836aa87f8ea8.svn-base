<%@ Page Language="C#" MasterPageFile="~/SiteParent.Master" AutoEventWireup="true" CodeBehind="SingleProjectAndFileList.aspx.cs"
...
<asp:Content ID="Content1" ContentPlaceHolderID="Head" runat="server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        //查看文件信息
        function btnLookFile() {
            if (Common.isRadioCheck()) {
                window.top.parent.closeMenu();
                var keyList = Common.getRadioKeyToArray();
                var url = "../WorkFlow/wjdjOther.aspx?SingleProjectID=" + keyList[0] + "&ModelBH=outhelp_to&WorkFlowID=0&rn=" + Math.random();
        ...
        <div class="content-box-content">
            <div class="tab-content default-tab" id="tab1">
                <div class="anl" id="anlButtonList">
                    <input type="button" id="MrcBtnLookSingle" title="查看工程详细信息" onclick="btnLookSingle()" value="工程信息" />
                    <input type="button" id="MrcBtnArchiveToFileList" title="查看文件清单及其它操作" onclick="btnLookFile()" value="文件清单" />
                </div>
                <table>
                    <thead>
                        <tr>
                            ...
                            <ItemTemplate>
                                <tr class='<%#(Container.ItemIndex%2==0)?"bg01":""%>'>
                                    ...
                                    <td><%#  DigiPower.Onlinecol.Standard.Web.PublicModel.getSingleProjectStatusNameByStatus(
                                            DigiPower.Onlinecol.Standard.Common.ConvertEx.ToInt(Eval("Status"))) %>                                              
                                    </td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
