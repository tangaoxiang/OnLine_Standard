...
namespace DigiPower.Onlinecol.Distribution.Service {
    ...
    public abstract class DbHelperOra {
        ...
        public static string connectionString = PubConstant.ConnectionStringOra;
        public DbHelperOra() {
        }
        ...
        public static DataSet Query(string SQLString) {
            using (OracleConnection connection = new OracleConnection(connectionString)) {
                ...
                try {
                    //string tSQLString = SQLString.ToLower();
                    //int iPos = tSQLString.IndexOf("isnull(");
                    //if (iPos > 0)
                    //{
                    //    string strNULL = SQLString.Substring(iPos, 7);
                    //    SQLString = SQLString.Replace(strNULL, "NVL(");
                    //}

                    SQLString = SQLString.Replace("ISNULL(", "NVL(");
                    connection.Open();
        ...
        public static int ExecuteSql(string SQLString, params OracleParameter[] cmdParms) {
            using (OracleConnection connection = new OracleConnection(connectionString)) {
                using (OracleCommand cmd = new OracleCommand()) {
                    try {
                        for (int i1 = 0; i1 < cmdParms.Length; i1++) {
                            if (cmdParms[i1].Value == null) {
                                cmdParms[i1].Value = DBNull.Value;
                            }
                        }
